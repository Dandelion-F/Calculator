import { useCounterMachine } from './useCounterMachine.js';
import { simpleBinaryOperList } from '../components/Counter/Operation/operation.ts';
...
document.addEventListener('keydown', (e) => {
    ...
    switch (key) {
        ...
        case 'ArrowDown':
            ...
            break;
        case 'Enter':
            if (classList.contains("operation-delete-btn")) {
                useCounterMachine.send({ type: 'DEL' });
            } else if (classList.contains("operation-equal-btn")) {
                useCounterMachine.send({ type: 'EQUAL', value: '=' });
            } else if (classList.contains('operation-number-btn')) {
                let type = btnId === '0' ? 'ZERO' : btnId === '.' ? 'POINT' : 'INT';
                useCounterMachine.send({ type, value: btnId });
            } else if (classList.contains('operation-operator-btn')) {
                let isDisabledBtn = (simpleBinaryOperList.findIndex(oper => oper === btnId) < 0) ? true : false;
                if (!isDisabledBtn) {
                    useCounterMachine.send({ type: 'OPER', value: btnId });
                }
            } else if (classList.contains('operation-reset-btn')) {
